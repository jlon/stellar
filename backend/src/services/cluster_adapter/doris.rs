...
impl ClusterAdapter for DorisAdapter {
    ...
    async fn list_user_permissions(&self, username: &str) -> ApiResult<Vec<crate::models::DbUserPermissionDto>> {
        tracing::debug!(
            "[Doris] list_user_permissions not fully implemented yet; returning empty list (username={})",
            username
        );
        Ok(Vec::new())
    }
    ...
    async fn list_db_accounts(&self) -> ApiResult<Vec<crate::models::DbAccountDto>> {
        tracing::debug!("[Doris] list_db_accounts not fully implemented yet; returning empty list");
        Ok(Vec::new())
    }

    async fn list_db_roles(&self) -> ApiResult<Vec<crate::models::DbRoleDto>> {
        tracing::debug!("[Doris] list_db_roles not fully implemented yet; returning empty list");
        Ok(Vec::new())
    }
}
