...
pub async fn list_incidents<DB: AppDb>(
    ...
    resolve_cluster_access(&state, &org_ctx, params.cluster_id).await?;
...
pub async fn get_incident<DB: AppDb>(
    ...
    resolve_incident_cluster(&state, &org_ctx, incident_id).await?;
    Ok(Json(
        state
            .agent_runtime_service
...
/// 校验 cluster 的组织归属；列表 API 只持有 cluster id 时共用。
#[app_db]
async fn resolve_cluster_access<DB: AppDb>(
    state: &Arc<AppState<DB>>,
    org_ctx: &OrgContext,
    cluster_id: i64,
) -> ApiResult<crate::models::cluster::Cluster> {
    if cluster_id <= 0 {
        return Err(ApiError::validation_error("Invalid cluster id"));
    }
    let cluster = state.cluster_service.get_cluster(cluster_id).await?;
    check_org_access(org_ctx, cluster.organization_id, "access Incidents")?;
    Ok(cluster)
}

/// 解析 Incident 归属集群，并先校验组织边界。
#[app_db]
async fn resolve_incident_cluster<DB: AppDb>(
    ...
    let cluster = state.cluster_service.get_cluster(cluster_id).await?;
    check_org_access(org_ctx, cluster.organization_id, "access this Incident")?;
    Ok(cluster)
}
