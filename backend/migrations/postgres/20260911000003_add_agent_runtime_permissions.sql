...
('api:agent:events:list', '查看事件列表', 'api', 'agent', 'events', 'GET /api/agent/events') ON CONFLICT DO NOTHING;

UPDATE permissions
SET parent_id = (SELECT id FROM permissions WHERE code = 'menu:agent')
WHERE code IN ('api:agent:incidents:list', 'api:agent:incidents:get', 'api:agent:incidents:investigate', 'api:agent:incidents:close', 'api:agent:events:list') ON CONFLICT DO NOTHING;
