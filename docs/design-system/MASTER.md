...
- 智能助手是“集群概览”后的直接入口，而非独立菜单分组；离开全量会话页后保留固定右下角无文字图标，点击展开现有 Nebular 右侧抽屉。抽屉与全量页必须共享当前会话和流式回合，不能各自维护会话状态。
...
- 弹窗打开、下拉面板、toast 通知、表格首屏行入场动效已在全站落地，iOS 风格（短时长、低位移、仅 opacity/transform）。表格行错峰仅前 8 行，其余直接显示。页面切换入场在 `one-column.layout.scss`（选择器必须 `::ng-deep .main-column__body > :not(router-outlet)`，投影内容不带组件 scoping 属性）。这些跨页动效由全局 `prefers-reduced-motion: reduce` 兜底；组件自有动效必须在组件 SCSS 中自行降级。
...
3. 动效只表达状态变化，无装饰性动画；正常、健康与历史数据默认静态展示，受控的计划可视化除外。
...
动效只能回答一个明确的操作问题：内容从何处出现或消失、用户的写操作是否已生效、异步工作是否仍在进行。不能回答该问题的动效一律不加。

- 正常健康、已完成作业和静态图表不得使用呼吸、脉冲、流光或循环位移；颜色、文案和图标表达状态。历史 Profile 的 DAG 流动仅可作为可切换的计划可视化辅助，必须提供明确开关和减弱动态下的静态回退，且不能暗示查询仍在运行。
- 持续循环只允许用于真实持续中的加载、进度、流式输出光标或活跃任务；任务结束后必须停止。待人工确认的卡片最多做 3 次短提示，随后保持静态。
- 页面已有全局进入、表格首屏、Dialog、Sheet 与 Toast 动效时，业务组件不得叠加第二层进入动画。高密度图表和表格刷新不做滚动揭示或逐项跳动。
- Kinetics 仅可作为交互语义参考；Angular/Nebular 页面优先复用现有 CSS 与 Nebular 机制，不为常规淡入、过渡或状态反馈引入动画依赖。

### 3.2 实现约束

- 微交互为 150~200ms，Dialog 为 240ms，Sheet 为 400ms；除持续工作反馈外，不得超过 400ms。空间或可见性变化仅使用 `opacity` 和 `transform`，不动画化布局尺寸、滚动位置、阴影或滤镜。
- 写操作使用确认弹窗、行内状态更新和成功/失败 Toast；需要帮助用户定位更新结果时，允许目标行一次性 160ms 背景淡显。
- 动效归属保持明确：`_overrides.scss` 只维护跨页 Nebular/Smart Table 基线；页面、插图和业务组件的动效及其降级规则放在自身 SCSS。

### 3.3 可达与验收

- 每个新增 CSS 动效必须在同一组件内提供 `@media (prefers-reduced-motion: reduce)`：移除入场、循环和非必要 transition；保留静态的最终可读状态。仅有真实状态意义的 loading、流式反馈也必须有文案或图标，不能只依赖运动传达信息。
- 涉及代码控制的逐字输出、自动滚动或焦点移动，必须读取 `matchMedia('(prefers-reduced-motion: reduce)')` 并切换为直接呈现或即时滚动。
- 验收至少覆盖默认与减弱动态两种设置，检查动效停止后内容仍完整可见、键盘焦点不丢失、加载/错误/空态语义不被隐藏；对运维页额外检查没有无限装饰循环和刷新抖动。
...
- 将功能前置条件、数据源降级等静态说明渲染为全宽高饱和 `nb-alert` 横幅。此类说明使用卡片内、正文最大 `46rem` 的低对比度图标提示行；`nb-alert` 仅用于真实错误、失败或需要立即处置的状态。
