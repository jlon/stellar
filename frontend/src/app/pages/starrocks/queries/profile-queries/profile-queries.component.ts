...
import {
  MetricThresholds,
  renderMetricBadge,
  parseStarRocksDuration,
} from "../../../../@core/utils/metric-badge";
import { renderLongText } from "../../../../@core/utils/text-truncate";
import { themeColor } from "../../../../@core/utils/theme-color";
import { retestDeltaLabel } from "../../../../@core/utils/retest-delta";
...
export class ProfileQueriesComponent implements OnInit, OnDestroy {
  ...
  private profileDurationThresholds: MetricThresholds = {
    warn: 120000,
    danger: 240000,
  ...
  @ViewChild("profileDetailDialog")
  profileDetailDialogTemplate: TemplateRef<any>;
  ...
  retestRuns: any[] = [];
  retestScanned = 0;
  retestAuditAvailable = false;
  retestWindowMinutes = 0;
  retestTruncated = false;
  retestLoading = false;
  applyingParam = "";
  ...
  private coreMetricsByType: { [key: string]: string[] } = {
    ...
    AGGREGATE: [
      ...
      "GroupByKeyCount",
      "StreamingTime",
      "HashTableIteratorTime",
    ],
    EXCHANGE: [
      "ShuffleRowsPerBuffer",
      "NetworkTime",
      "WaitTime",
      "OverallThroughput",
      "PeakBufferUsage",
  ...
  metricDescriptions: { [key: string]: string } = {
    ...
    QueryCumulativeOperatorTime:
      "Operator执行的总时间，为Operator时间百分比的分母",
    ...
    // ============================================
    DriverTotalTime:
      "Driver消耗的总时间。DriverTotalTime = ActiveTime + PendingTime + ScheduleTime",
    ActiveTime:
      "Driver执行时间。ActiveTime = ∑OperatorTotalTime + OverheadTime",
    ...
    SendDataTime:
      "发送数据的本地时间消耗，包括序列化和压缩数据的时间，以及将任务提交到发送者队列的时间",
  ...
  constructor() {
    ...
    const routeClusterId = parseInt(
      this.route.snapshot.paramMap.get("clusterId") || "0",
      10,
    );
  ...
  loadAnalysis(queryId: string, refresh = false): void {
    ...
    this.nodeService.analyzeProfile(queryId, refresh).subscribe({
      next: (data) => {
        ...
        if (
          data.llm_analysis?.available &&
          data.llm_analysis?.status === "pending"
        ) {
      ...
      error: (err) => {
        ...
        this.analysisError =
          "分析失败: " + (err.error?.message || err.message || "未知错误");
        this.analysisLoading = false;
        this.profileDetailLoading = false;
      },
    });
   ...
   */
  /** 同一 SQL 指纹的执行序列：处置前后耗时对比的事实来源。 */
  loadRetest(queryId: string): void {
    if (!queryId || this.retestLoading) {
      return;
    }
    this.retestLoading = true;
    this.nodeService
      .getProfileRetest(queryId)
      .pipe(takeUntil(this.destroy$), timeout(20000))
      .subscribe({
        next: (data) => {
          this.retestRuns = data?.Runs || [];
          this.retestScanned = data?.scanned || 0;
          this.retestAuditAvailable = Boolean(data?.audit_available);
          this.retestWindowMinutes = data?.window_minutes || 0;
          this.retestTruncated = Boolean(data?.truncated);
  ...
  /** 复测数据来源说明：审计日志（全量）或 profile 窗口（滚动）。 */
  retestSourceLabel(): string {
    if (!this.retestRuns.length) {
      return "";
    }
    if (this.retestAuditAvailable) {
      const batch = this.retestTruncated ? "，仅最近一批" : "";
      return `来源：审计日志 / 最近 ${this.retestWindowMinutes} 分钟${batch}`;
    }
    return `来源：profile 窗口 / ${this.retestScanned} 条`;
  }

  /** 诊断那次与最近一次执行的耗时对比（逻辑见 utils/retest-delta）。 */
  retestDelta(): string {
    return retestDeltaLabel(this.retestRuns);
  }

  /** 应用规则引擎给出的参数建议；GLOBAL 影响整集群，先确认再执行。 */
  applyParameter(param: any): void {
    const name = String(param?.name || "").trim();
    const value = String(param?.recommended || "").trim();
    if (!name || !value) {
      return;
    }
    const scope =
      String(param?.param_type || "").toLowerCase() === "session"
        ? "SESSION"
        : "GLOBAL";
    const execute = () => this.updateVariable(name, value, scope);
    if (scope === "GLOBAL") {
      this.confirmDialogService
        .confirm(
          `修改全局参数 ${name}`,
          `该变更对当前集群全局生效：${param?.command || `${name} = ${value}`}`,
          "执行",
          "取消",
          "primary",
        )
  ...
  private updateVariable(name: string, value: string, scope: string): void {
    this.applyingParam = name;
    this.nodeService
      ...
      .subscribe({
        next: () => {
          this.applyingParam = "";
          this.toastrService.success(`${name} 已设置为 ${value}`, "参数已应用");
          if (this.currentQueryId) {
            this.loadAnalysis(this.currentQueryId, true);
            // 处置后立刻重新取同一查询的执行序列，让"有没有变快"当场可见
            this.retestRuns = [];
            this.loadRetest(this.currentQueryId);
          }
          this.cdr.markForCheck();
        },
        error: (err) => {
  ...
  private loadLLMAnalysis(
    ...
    this.nodeService
      .enhanceProfileWithLLM(this.clusterId, queryId, payload)
      .subscribe({
        next: (llmData) => {
          ...
          if (this.analysisData) {
            ...
            if (this.analysisData.llm_analysis) {
              this.analysisData.llm_analysis.elapsed_time_ms =
                this.llmAnalysisElapsedTime;
            }
          }
  ...
  buildGraph(tree: any): void {
    ...
    this.graphNodes = nodeList.map((node: any) => {
      ...
      const x =
        layoutNode && isFinite(layoutNode.x) ? Math.round(layoutNode.x) : 0;
      const y =
        layoutNode && isFinite(layoutNode.y) ? Math.round(layoutNode.y) : 0;
      const width =
        layoutNode && isFinite(layoutNode.width)
          ? Math.round(layoutNode.width)
          : 220;
      const height =
        layoutNode && isFinite(layoutNode.height)
          ? Math.round(layoutNode.height)
          : 95;
    ...
    this.graphEdges = g.edges().map((e: any, index: number) => {
      ...
      let labelFormatted =
        rows > 0 ? "Rows: " + Number(rows).toLocaleString() : "";
    ...
    const maxX = Math.max(
      ...this.graphNodes.map((n: any) => (n.x || 0) + (n.width || 0) / 2),
    );
    const maxY = Math.max(
      ...this.graphNodes.map((n: any) => (n.y || 0) + (n.height || 0) / 2),
    );
  ...
  private updateEdgesAfterRender(): void {
    ...
    nodeElements.forEach((el: Element) => {
      ...
      if (nodeId) {
        actualHeights.set(
          nodeId,
          el.getBoundingClientRect().height / this.zoomLevel,
        );
      }
    });
    ...
    this.graphEdges = this.graphEdges.map((edge) => {
      ...
      if (sourceNode && targetNode && edge.points && edge.points.length >= 2) {
        ...
        const originalEndY =
          targetNode.y - targetNode.height / 2 + targetActualH;
  ...
  get graphTransform(): string {
    ...
    return this.zoomLevel === 1
      ? translate
      : `${translate} scale(${this.zoomLevel})`;
  }

  centerGraph(): void {
    const viewport = document.querySelector(
      ".dag-layout .graph-viewport",
    ) as HTMLElement;
    if (viewport && this.graphWidth > 0 && this.graphHeight > 0) {
      ...
      this.translateY = Math.round(
        (vh - this.graphHeight * this.zoomLevel) / 2,
      );
    }
  }
  ...
  get top10NodesByTime(): any[] {
    ...
    return [...this.graphNodes]
      .filter(
        (n) => n.time_percentage !== undefined && n.time_percentage !== null,
      )
  ...
  get top10NodesByMemory(): any[] {
    ...
    return [...this.graphNodes]
      .sort(
        (a, b) =>
          (b.metrics?.memory_usage || 0) - (a.metrics?.memory_usage || 0),
      )
      .slice(0, 10);
  }
  ...
  getScanTimePercentage(): number {
    const scanMs =
      this.analysisData?.summary?.query_cumulative_scan_time_ms || 0;
    const totalMs =
      this.analysisData?.summary?.query_cumulative_operator_time_ms ||
      this.analysisData?.summary?.query_execution_wall_time_ms ||
      1;
  ...
  getScheduleTimePercentage(): number {
    const scheduleMs =
      this.analysisData?.summary?.query_peak_schedule_time_ms || 0;
    const totalMs =
  ...
  getNetworkTimePercentage(): number {
    const networkMs =
      this.analysisData?.summary?.query_cumulative_network_time_ms || 0;
    const totalMs =
      this.analysisData?.summary?.query_execution_wall_time_ms || 1;
  ...
  hasIoMetrics(): boolean {
    ...
    return !!(
      s?.io_seek_time ||
      s?.local_disk_read_io_time ||
      s?.remote_read_io_time
    );
  }
  ...
  hasIoStatistics(): boolean {
    const s = this.analysisData?.summary;
    return !!(
      s?.total_raw_rows_read ||
      s?.total_bytes_read ||
      s?.pages_count_memory ||
      s?.pages_count_local_disk ||
      s?.pages_count_remote ||
      s?.result_rows ||
      s?.result_bytes
    );
  }
  ...
  hasAnyMetric(): boolean {
    ...
    if (!s) return false;
    return !!(
      s.query_execution_wall_time ||
      s.query_cumulative_operator_time ||
      s.query_cumulative_cpu_time ||
      s.query_cumulative_scan_time ||
      s.query_cumulative_network_time ||
      s.query_peak_schedule_time ||
      s.result_deliver_time ||
      s.query_sum_memory_usage ||
      s.query_spill_bytes ||
  ...
  getDataCacheTooltip(): string {
    ...
    const hitRate =
      s.datacache_hit_rate != null
        ? (s.datacache_hit_rate * 100).toFixed(1)
        : "0";

    return (
      `存算分离架构 DataCache 命中率\n` +
      `本地缓存读取: ${local}\n` +
      `远程存储读取: ${remote}\n` +
      `命中率: ${hitRate}%\n` +
      `(命中率 >= 70% 为健康)`
  ...
  getExecutionTimeBreakdown(): {
    items: { label: string; time: string; percent: number; color: string }[];
  } | null {
    ...
    const cpuTimeStr =
      allMetrics["__MAX_OF_OperatorTotalTime"] ||
      allMetrics["OperatorTotalTime"] ||
      "0ns";
  ...
  getNodeDetailMetrics(): {
    category: string;
    metrics: { key: string; value: any; description?: string }[];
  }[] {
    if (!this.selectedNode) return [];
    ...
    if (
      this.selectedNode.unique_metrics &&
      Object.keys(this.selectedNode.unique_metrics).length > 0
    ) {
    ...
    if (
      this.selectedNode.aggregated_metrics &&
      Object.keys(this.selectedNode.aggregated_metrics).length > 0
    ) {
  ...
  getNodeIoPercentage(): number {
    ...
    const ioTimeStr =
      this.selectedNode.aggregated_metrics?.IOTime ||
      this.selectedNode.unique_metrics?.IOTime ||
      this.selectedNode.metrics?.IOTime ||
      0;
    const ioTimeNs = this.parseTimeToNs(ioTimeStr);

    // Get total operator time
    const totalTimeNs =
      this.selectedNode.metrics?.operator_total_time_ns ||
      this.parseTimeToNs(this.selectedNode.metrics?.operator_total_time) ||
      1;
  ...
  getNodeFragmentInfo(): any | null {
    if (!this.selectedNode?.fragment_id || !this.analysisData?.fragments)
      return null;
    return this.analysisData.fragments.find(
      (f: any) => f.id === this.selectedNode.fragment_id,
    );
  }
  ...
  getFragmentMetrics(): { key: string; value: any }[] {
    ...
    if (fragment.backend_addresses?.length) {
      result.push({
        key: "BackendAddresses",
        value: fragment.backend_addresses.join(", "),
      });
    }
  ...
  getNodePipelineFromFragments(): any | null {
    ...
    return fragment.pipelines?.find(
      (p: any) => p.id === this.selectedNode.pipeline_id,
    );
  }
  ...
  getNodeOperatorsFromPipeline(): any[] {
    ...
    return pipeline.operators.filter(
      (op: any) => op.plan_node_id === planNodeId,
    );
  }
  ...
  getTotalRecommendationsCount(): number {
    const llmCount = this.hasLLMAnalysis()
      ? this.getRootCauseAnalysis()?.recommendations?.length || 0
      : 0;
  ...
  getEdgeLabelPosition(points: { x: number; y: number }[]): {
    x: number;
    y: number;
  } {
  ...
  exportDagAsPng(): void {
    const graphContent = document.querySelector(
      ".graph-content",
    ) as HTMLElement;
    if (!graphContent) {
      this.toastrService.warning(
        this.i18n.instant("无法找到图表内容"),
        this.i18n.instant("导出失败"),
      );
      return;
    }
        ...
        html2canvas(graphContent, {
          ...
          .then((canvas) => {
            ...
            this.toastrService.success(
              this.i18n.instant("图表已导出"),
              this.i18n.instant("导出成功"),
            );
          })
          .catch((err) => {
            console.error("Export failed:", err);
            this.toastrService.danger(
              this.i18n.instant("导出图片失败"),
  ...
  startPan(event: MouseEvent): void {
    ...
    if (event.button === 0) {
      ...
      const viewport = document.querySelector(
        ".dag-center-panel",
      ) as HTMLElement; // Use panel cursor
  ...
  getProgressColor(node: any): string {
    ...
    if (rank > 0 || this.isScanNode(node) || this.isJoinNode(node))
      return themeColor("--color-warning-default", "#ffaa00");
    return themeColor("--text-hint-color", "#8f9bb3");
  }

  // Toggle functions for right panel sections
  toggleTop10(): void {
    this.isTop10Collapsed = !this.isTop10Collapsed;
  }
  toggleSummary(): void {
    this.isSummaryCollapsed = !this.isSummaryCollapsed;
  }
  toggleDiagnosis(): void {
    this.isDiagnosisCollapsed = !this.isDiagnosisCollapsed;
  }
  ...
  copyProfileToClipboard(): void {
    ...
    if (navigator.clipboard && window.isSecureContext) {
      navigator.clipboard
        .writeText(this.currentProfileDetail)
        .then(() => {
          this.toastrService.success(
            this.i18n.instant("Profile 内容已复制到剪贴板"),
            this.i18n.instant("复制成功"),
          );
        })
        .catch((err) => {
