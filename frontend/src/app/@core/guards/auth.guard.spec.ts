...
describe('AuthGuard', () => {
  ...
  it('redirects unauthenticated users to login with normalized returnUrl', () => {
    ...
    authServiceMock.getLoginCommands.and.returnValue(['/', 'stellar', 'auth', 'login']);
    const state = createState('/stellar/pages/starrocks/dashboard');
    ...
    expect(serialized).toBe('/stellar/auth/login?returnUrl=%2Fpages%2Fstarrocks%2Fdashboard');
  });
});
